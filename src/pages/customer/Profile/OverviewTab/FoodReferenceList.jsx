...
export default function FoodReferenceList({ customerDetails, vegetarianTypes, setPrefill, setEditMode, setDialogOpen, setUserDeclined }) {
  ...
  const vegetarianLabel = vegetarianTypes.find(v => v.value === reference?.vegetarianType)?.label || 'Not specified'

  // Render list as Chips for nicer UI
  const renderChips = (arr, key, color = 'default') => {
    if (!Array.isArray(arr) || arr.length === 0) {
      return <Typography sx={{ color: 'text.disabled' }}>No information yet</Typography>
    }
    return (
      ...
      {customerDetails?.customerReference ? (
          ...
          <Box sx={{ backgroundColor: 'white', borderRadius: 2, boxShadow: 2, overflow: 'hidden' }}>
            ...
            <Box sx={{ p: { xs: 2, sm: 3 } }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, flexWrap: 'wrap' }}>
                <Typography sx={{ fontWeight: 'bold', minWidth: 140 }}>🥗 Dietary Preferences:</Typography>
                <Chip
                ...
                {(reference.canEatEggs || reference.canEatDairy) && (
                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                    {reference.canEatEggs && (
                      <Chip label="Can eat eggs" color="success" size="small" icon={<span>🥚</span>} />
                    )}
                    {reference.canEatDairy && (
                      <Chip label="Can eat dairy" color="success" size="small" icon={<span>🥛</span>} />
                    )}
                  </Box>
                )}
              </Box>
              ...
              {reference.note && (
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, flexWrap: 'wrap' }}>
                  <Typography sx={{ fontWeight: 'bold', minWidth: 140 }}>💭 Notes:</Typography>
              ...
              <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ ml: 1 }}>
                  {Array.isArray(reference.allergies) && reference.allergies.length > 0
                    ...
                    : <Typography sx={{ color: '#43a047', fontWeight: 500 }}>No allergies</Typography>}
                </Box>
              </Box>

              {/* Protein yêu thích */}
              <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2, flexWrap: 'wrap' }}>
                <Typography sx={{ fontWeight: 'bold', minWidth: 140 }}>🥩 Favorite Proteins:</Typography>
                <Box sx={{ ml: 1 }}>
              ...
              {/* Carbs yêu thích */}
              <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 2, flexWrap: 'wrap' }}>
                <Typography sx={{ fontWeight: 'bold', minWidth: 140 }}>🍚 Favorite Carbs:</Typography>
                <Box sx={{ ml: 1 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 1, flexWrap: 'wrap' }}>
                <Typography sx={{ fontWeight: 'bold', minWidth: 140 }}>🥬 Favorite Vegetables:</Typography>
                <Box sx={{ ml: 1 }}>
        ...
        <Box sx={{ textAlign: 'center', py: 8, backgroundColor: 'white' }}>
          ...
          <Typography variant="body1" sx={{ mb: 3, color: 'text.secondary' }}>
            Share your food preferences so we can serve you better
          </Typography>
          <Button
            variant="contained"
            onClick={() => {
