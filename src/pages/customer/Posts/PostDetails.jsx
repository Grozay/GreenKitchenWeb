  ...
  if (loading) return (
    <Box>
      ...
      <Box sx={{
        ...
        <Box sx={{ mt: 1, py: 1 }}>
          <Button startIcon={<ArrowBackIosIcon fontSize='small' />} variant="text" size="medium" onClick={() => navigate('/forum')}>Back to Forum</Button>
        </Box>
        <Box sx={{ p: 3 }}>
          ...
          {relatedPosts.length > 0 && (
            <Box sx={{ mt: 4 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, overflowX: 'auto', pb: 1 }}>
                {relatedPosts.map(r => (
                  <ButtonBase key={r.id} onClick={() => navigate(`/forum/${r.slug || r.id}`)} sx={{ minWidth: 260 }}>
                    <Card sx={{ width: 260 }}>
