...
const WeeklyTrendingMenus = ({ customBlue }) => {
  ...
  return (
    <Card sx={{ p: 2, boxShadow: 0 }}>
      ...
      {menus.length === 0 ? (
        <Typography color="textSecondary" fontSize={13}>No data available</Typography>
      ) : (
        menus.slice(0, 5).map(menu => ( // Limit to 5
          <Box key={menu.id} display="flex" alignItems="center" mb={1}>
            ...
            <Typography fontWeight={700} sx={{ color: customBlue }}>${Math.round(menu.price).toLocaleString()}</Typography>
